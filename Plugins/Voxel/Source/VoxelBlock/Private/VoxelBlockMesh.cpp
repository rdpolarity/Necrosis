...
int64 FVoxelBlockMesh::GetAllocatedSize() const
{
	return
...
void FVoxelBlockMesh::Initialize_GameThread()
{
...
void FVoxelBlockMesh::Initialize_RenderThread(ERHIFeatureLevel::Type FeatureLevel)
{
	VOXEL_FUNCTION_COUNTER();
...
void FVoxelBlockMesh::Destroy_RenderThread()
{
	VOXEL_FUNCTION_COUNTER();
	check(IsInRenderingThread());
...
bool FVoxelBlockMesh::Draw_RenderThread(
...
const FRayTracingGeometry* FVoxelBlockMesh::DrawRaytracing_RenderThread(const FPrimitiveSceneProxy& Proxy, FMeshBatch& MeshBatch) const
{
	MeshBatch.VertexFactory = VertexFactory.Get();
