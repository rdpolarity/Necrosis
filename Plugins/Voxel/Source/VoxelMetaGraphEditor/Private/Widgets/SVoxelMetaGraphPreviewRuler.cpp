...
#include "SVoxelMetaGraphPreviewRuler.h"
...
void SPreviewRuler::Construct(const FArguments& InArgs)
{
	...
	[
		...
		[
			...
			{
				const float TextWidth = FSlateApplication::Get().GetRenderer()->GetFontMeasureService()->Measure(DistanceText, FEditorAppStyle::GetFontStyle("PropertyWindow.FilterFont")).X / 2.f;
				const FVector2D UnitVector = (EndPosition - StartPosition).GetSafeNormal();
			...
			.Size_Lambda([this]
			{
				return FSlateApplication::Get().GetRenderer()->GetFontMeasureService()->Measure(DistanceText, FEditorAppStyle::GetFontStyle("PropertyWindow.FilterFont")) + 3.f;
			})
			.HAlign(HAlign_Fill)
			.VAlign(VAlign_Fill)
			[
				...
				.Visibility(EVisibility::Collapsed)
				.Font(FEditorAppStyle::GetFontStyle("PropertyWindow.FilterFont"))
				.Text_Lambda([this]
				{
