...
#include "SVoxelMetaGraphNodeVariable.h"
#include "SGraphPin.h"
#include "SCommentBubble.h"
#include "GraphEditorSettings.h"

void SVoxelMetaGraphNodeVariable::Construct(const FArguments& InArgs, UVoxelGraphNode* InNode)
{
...
void SVoxelMetaGraphNodeVariable::UpdateGraphNode()
{
	VOXEL_FUNCTION_COUNTER();
	...
	[
		...
		[
			...
			[
				SNew(SImage)
				.Image(FEditorAppStyle::GetBrush("Graph.VarNode.ColorSpill"))
				.ColorAndOpacity(this, &SVoxelMetaGraphNodeVariable::GetVariableColor)
			]
			+ SOverlay::Slot()
			[
				SNew(SImage)
...
const FSlateBrush* SVoxelMetaGraphNodeVariable::GetShadowBrush(bool bSelected) const
{
	return bSelected ? FEditorAppStyle::GetBrush(TEXT("Graph.VarNode.ShadowSelected")) : FEditorAppStyle::GetBrush(TEXT("Graph.VarNode.Shadow"));
}

FSlateColor SVoxelMetaGraphNodeVariable::GetVariableColor() const
{
	return GraphNode->GetNodeTitleColor();
}

TSharedRef<SWidget> SVoxelMetaGraphNodeVariable::UpdateTitleWidget(FText InTitleText) const
{
	...
	return
		SNew(STextBlock)
		.TextStyle(FEditorAppStyle::Get(), "Graph.Node.NodeTitle")
		.Text(InTitleText);
}