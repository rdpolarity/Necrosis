	...
	{
		{
			...
			const void* BufferData = RHILockBuffer(PositionVertexBuffer.VertexBufferRHI, 0, NumBytes, RLM_ReadOnly);
...

		{
			...
			const void* BufferData = RHILockBuffer(IndexBuffer.IndexBufferRHI, 0, NumBytes, EResourceLockMode::RLM_ReadOnly);
			if (bIs32Bit)
			{
		...
		
		{
			...
			if (StaticMeshVertexBuffer.GetUseHighPrecisionTangentBasis())
			{
				...
				const void* BufferData = RHILockBuffer(
	...
	bool bMultiThreaded)
{
	...
	// Propagate distances
	FVoxelDistanceFieldUtilities::JumpFlood(Size, OutSurfacePositions, Device, bMultiThreaded);
