...
IMPLEMENT_GLOBAL_SHADER(FVoxelJumpFloodCS, "/Engine/Plugins/Voxel/Private/DistanceField.usf", "ExpandDistanceField", SF_Compute);
...
void FVoxelDistanceFieldShaderHelper::StartCompute(const FIntVector& Size, const TSharedRef<TArray<FVector3f>>& InOutData, int32 MaxPasses_Debug)
{
	VOXEL_FUNCTION_COUNTER();
	check(IsInGameThread());
	...
	ENQUEUE_RENDER_COMMAND(VoxelDistanceFieldCompute)(
		MakeWeakPtrLambda(this, [=](FRHICommandListImmediate& RHICmdList)
		{
	...
	int32 MaxPasses_Debug)
{
	...
	if (AllocatedSize != Size)
	{
		...
		SrcBuffer.Initialize(TEXT("SrcBuffer"), sizeof(float), 3 * Num, PF_R32_FLOAT);
		DstBuffer.Initialize(TEXT("DstBuffer"), sizeof(float), 3 * Num, PF_R32_FLOAT);
	}
