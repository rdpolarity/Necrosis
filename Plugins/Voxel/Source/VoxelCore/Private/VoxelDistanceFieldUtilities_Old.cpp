...
#include "VoxelDistanceFieldShader.h"

void FVoxelDistanceFieldUtilities::JumpFlood(
	const FIntVector& Size, 
	TArray<FVector3f>& InOutSurfacePositions, 
	...
	const FVoxelIntBox* InBounds)
{
	...
	{
		ensure(!InBounds); // Not supported yet

		const auto DataPtr = MakeShared<TArray<FVector3f>>(MoveTemp(InOutSurfacePositions));

		const auto Helper = MakeShared<FVoxelDistanceFieldShaderHelper>();
		Helper->StartCompute(Size, DataPtr);
...
void FVoxelDistanceFieldUtilities::GetDistancesFromSurfacePositions(
	const FIntVector& Size,
	TArrayView<const FVector3f> SurfacePositions,
	TArrayView<float> InOutDistances,
	const FVoxelIntBox* InBounds)
{
	...
	for (int32 Z = Bounds.Min.Z; Z < Bounds.Max.Z; Z++)
	{
		...
		{
			float* RESTRICT const DistancePtr = &FVoxelUtilities::Get3D(InOutDistances, Size, 0, Y, Z);
			const FVector3f* RESTRICT const SurfacePositionPtr = &FVoxelUtilities::Get3D(SurfacePositions, Size, 0, Y, Z);
			for (int32 X = Bounds.Min.X; X < Bounds.Max.X; X++)
			{
				...
				checkVoxelSlow(&Distance == &FVoxelUtilities::Get3D(InOutDistances, Size, X, Y, Z));
				checkVoxelSlow(&SurfacePosition == &FVoxelUtilities::Get3D(SurfacePositions, Size, X, Y, Z));
	...
	FVoxelOptionalIntBox* OutBounds)
{
	...
	const auto Body = [&](auto ComputeOutBounds, const int32 Z)
	{
		for (int32 Y = 0; Y < Size.Y; Y++)
		{
			...
			int32 DensitiesIndex = FVoxelUtilities::Get3DIndex(DensitiesSize, PositionYZ + 1);
			int32 OutIndex = FVoxelUtilities::Get3DIndex(Size, PositionYZ);
...
#define	CheckNeighbor(bIsNegative, DX, DY, DZ) \
				{ \
					...
					checkVoxelSlow(NeighborIndex == FVoxelUtilities::Get3DIndex(DensitiesSize, FIntVector(X, Y, Z) + FIntVector(DX, DY, DZ) + 1)); \
	...
	bool bParallel)
{
	...
	const auto DoWork = [&](auto StaticStep, int32 Z)
	{
		for (int32 Y = Bounds.Min.Y; Y < Bounds.Max.Y; Y++)
		{
			int32 Index = FVoxelUtilities::Get3DIndex(Size, FIntVector(Bounds.Min.X, Y, Z));
			for (int32 X = Bounds.Min.X; X < Bounds.Max.X; X++)
			{
				ON_SCOPE_EXIT
				{
...
#define CheckNeighbor_DX(DX, DY, DZ) \
				{ \
					...
					checkVoxelSlow(NeighborIndex == FVoxelUtilities::Get3DIndex(Size, FIntVector(X, Y, Z) + FIntVector(DX * StaticStep, DY * StaticStep, DZ * StaticStep))); \
