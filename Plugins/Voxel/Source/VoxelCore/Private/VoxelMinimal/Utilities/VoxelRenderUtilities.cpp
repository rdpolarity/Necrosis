	...
	const TCHAR* Name)
{
	...
	const TSharedRef<FVoxelRDGExternalBuffer> Result = MakeShared<FVoxelRDGExternalBuffer>();
	...
	FResourceArrayInterface* ResourceArray)
{
	...
	FRDGBufferDesc Desc;
	Desc.Usage = BUF_Static | BUF_UnorderedAccess | BUF_ShaderResource | AdditionalFlags;

#if VOXEL_ENGINE_VERSION < 501
	Desc.UnderlyingType = FRDGBufferDesc::EUnderlyingType::VertexBuffer;
#else
	Desc.Usage |= BUF_VertexBuffer;
#endif

	Desc.BytesPerElement = BytesPerElement;
	Desc.NumElements = NumElements;
	ensure(Desc.NumElements <= MAX_int32);
	ensure(Desc.UE_501_SWITCH(GetTotalNumBytes, GetSize)() < MAX_uint32);
	...
	{
		Buffer = RHICreateBuffer(Desc.UE_501_SWITCH(GetTotalNumBytes, GetSize)(), Desc.Usage, CreateInfo);
	}
	else
	{
		// Buggy
	...
	Result->PooledBuffer = new FRDGPooledBuffer(Buffer, Desc, Desc.UE_501_SWITCH(GetTotalNumBytes, GetSize)(), Name);
	...
	FResourceArrayInterface* ResourceArray)
{
...
#else
	Desc.Usage |= BUF_StructuredBuffer;
#endif
	...
	FRHIResourceCreateInfo CreateInfo(Name, ResourceArray);
	const FBufferRHIRef Buffer = RHICreateStructuredBuffer(BytesPerElement, Desc.UE_501_SWITCH(GetTotalNumBytes, GetSize)(), Desc.Usage, CreateInfo);
	Result->PooledBuffer = new FRDGPooledBuffer(Buffer, Desc, Desc.UE_501_SWITCH(GetTotalNumBytes, GetSize)(), Name);
	...
	EBufferUsageFlags AdditionalFlags)
{
	...
	return Create(BytesPerElement, Num, Format, Name, AdditionalFlags, &ResourceArray);
}
...
FRHIShaderResourceView* FVoxelRDGExternalBuffer::GetSRV() const
{
	FRDGBufferSRVDesc Desc;
	Desc.BytesPerElement = BytesPerElement;
	Desc.Format = Format;
	return PooledBuffer->GetOrCreateSRV(Desc);
}

FRHIUnorderedAccessView* FVoxelRDGExternalBuffer::GetUAV() const
{
	FRDGBufferUAVDesc Desc;
	Desc.Format = Format;
	return PooledBuffer->GetOrCreateUAV(Desc);
}

void FVoxelRDGExternalBuffer::Resize(uint32 NewNumElements, bool bCopyData)
{
	VOXEL_FUNCTION_COUNTER();
	...
	{
		NewBuffer = Create(BytesPerElement, NewNumElements, Format, Name, PooledBuffer->Desc.Usage);
	}
	else
	{
		ensure(Type == EType::StructuredBuffer);
		NewBuffer = CreateStructured(BytesPerElement, NewNumElements, Name, PooledBuffer->Desc.Usage);
	}
...
void FVoxelRDGExternalBuffer::ResizeIfNeeded(uint32 NewNumElements, bool bCopyData, float GrowScale)
{
	ensure(GrowScale >= 1);
	...
	Resize(FMath::Max<uint32>(NewNumElements, NewNumElements * GrowScale), bCopyData);
}
...
{
	mutable TArray<TRefCountPtr<FRDGPooledBuffer>> PooledBuffers;
};
...
{
	if (!GraphBuilder.Blackboard.Get<FVoxelRDGBufferBlackboard>())
	{
		GraphBuilder.Blackboard.Create<FVoxelRDGBufferBlackboard>();
	}
	GraphBuilder.Blackboard.GetChecked<FVoxelRDGBufferBlackboard>().PooledBuffers.Add(ExternalBuffer.PooledBuffer.Get());
}
...
void ProcessVoxelShaderStatsScopes(FRDGBuilder& GraphBuilder)
{
	...
	for (int32 Index = 0; Index < GVoxelShaderStatsScopes.Num(); Index++)
	{
		...
		uint64 StartTime = 0;
		uint64 EndTime = 0;
		if (!RHICmdList.GetRenderQueryResult(Scope.TimeQueryEnd.GetQuery(), EndTime, false) || 
			!RHICmdList.GetRenderQueryResult(Scope.TimeQueryStart.GetQuery(), StartTime, false))
		{
			continue;
		}
...
FVoxelGPUBufferReadback::~FVoxelGPUBufferReadback()
{
	...
	const TSharedRef<TUniquePtr<FRHIGPUBufferReadback>> ReadbackRef = MakeSharedCopy(MakeUniqueCopy(Readback));
	...
	int64 Offset)
{
	...
	VOXEL_SCOPE_COUNTER_STRING(Buffer->GetName().ToString());

	void* Data = RHILockBuffer(Buffer, Offset, ArrayTypeSize * ArrayNum, RLM_WriteOnly);
	if (!ensure(Data))
	{
		return;
	}
...
void FVoxelRenderUtilities::UpdateBuffer(FRDGBuilder& GraphBuilder, FRDGBufferRef Buffer, const void* ArrayData, int64 ArrayTypeSize, int64 ArrayNum, int64 Offset, TSharedPtr<FVirtualDestructor> KeepAlive)
{
		...
		[=](FRHICommandListImmediate& RHICmdList)
	{
		...
		{
			UpdateBuffer(Buffer->GetRHI(), ArrayData, ArrayTypeSize, ArrayNum, Offset);
		}
	});
}
...
void FVoxelRenderUtilities::CopyBuffer(FRDGBuilder& GraphBuilder, TSharedPtr<FVoxelGPUBufferReadback>& Readback, FRDGBufferRef Buffer)
{
	...
	if (!Readback)
	{
		Readback = MakeShareable(new FVoxelGPUBufferReadback(Buffer->Name));
	}
	else
	{
	...
	const uint32 NumBytes = Buffer->Desc.UE_501_SWITCH(GetTotalNumBytes, GetSize)();
...
void FVoxelRenderUtilities::AsyncCopyTexture(
	...
	const TSharedRef<const TArray<uint8>>& Data,
	const FSimpleDelegate& OnComplete)
{
	...
	if (!GRHISupportsAsyncTextureCreation)
	{
		ENQUEUE_RENDER_COMMAND(AsyncCopyTexture)([=](FRHICommandListImmediate& RHICmdList)
		{
			...
			const TRefCountPtr<FRHITexture2D> UploadTextureRHI = RHICreateTexture2D(
				SizeX,
				SizeY,
				Format,
				1,
	...
	Async(EAsyncExecution::ThreadPool, [=]
	{
		...
		TArray<void*> MipData;
		MipData.Add(VOXEL_CONST_CAST(Data->GetData()));
...
{
	mutable TQueue<TFunction<void()>, EQueueMode::Mpsc> Queue;
};
RDG_REGISTER_BLACKBOARD_STRUCT(FVoxelGraphBuilderKeepAliveBlackboard);

void FVoxelRenderUtilities::KeepAlive(FRDGBuilder& GraphBuilder, TFunction<void()> Lambda)
{
	ensure(IsInRenderingThread());

	if (!GraphBuilder.Blackboard.Get<FVoxelGraphBuilderKeepAliveBlackboard>())
	{
		GraphBuilder.Blackboard.Create<FVoxelGraphBuilderKeepAliveBlackboard>();
	}

	GraphBuilder.Blackboard.GetChecked<FVoxelGraphBuilderKeepAliveBlackboard>().Queue.Enqueue(MoveTemp(Lambda));
}