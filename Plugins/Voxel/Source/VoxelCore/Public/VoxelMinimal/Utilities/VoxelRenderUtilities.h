...
public:
		...
		const TCHAR* Name)
	{
		...
		return FVoxelRDGExternalBuffer::Create(Array.GetTypeSize(), Array.Num(), Format, Name, BUF_None, &ResourceArray);
	}
...
public:
	...
	void Resize(uint32 NewNumElements, bool bCopyData);
	void ResizeIfNeeded(uint32 NewNumElements, bool bCopyData, float GrowScale = 1.25f);
...
#define DECLARE_VOXEL_SHADER_NAMESPACE(DebugName, Folder) \
	...
	constexpr const TCHAR* GVoxelShaderFolder = TEXT("/Engine/Plugins/Voxel/Private/" Folder); \
...
#define BEGIN_VOXEL_SHADER(Name, InShaderFrequency) \
	...
	{ \
		...
		public: \
			...
			{ \
			    static const FString Filename = FString::Printf(TEXT("%s/%s.usf"), GVoxelShaderFolder, TEXT(#Name)); \
				return *Filename; \
			} \
...
#define INTERNAL_BEGIN_VOXEL_SHADER_CALL(Name) \
	{ \
		...
		FRDGGPUStatScopeGuard GpuStatScope(GraphBuilder, STATIC_FNAME(#Name), {}, nullptr, &DrawCallCategoryName.Counters); \
...
FORCEINLINE FBufferRHIRef RHICreateBuffer(uint32 Size, EBufferUsageFlags Usage, FRHIResourceCreateInfo& CreateInfo)
{
	return RHICreateVertexBuffer(Size, Usage, CreateInfo);
}
...
public:
	...
	static void UpdateBuffer(FRDGBuilder& GraphBuilder, FRDGBufferRef Buffer, const TSharedRef<KeepAliveType>& KeepAlive, const ArrayType& Array, int64 Offset = 0)
	{
		...
		const TSharedRef<FKeepAlive> KeepAlivePtr = MakeShared<FKeepAlive>();
	...
	static void UpdateBuffer(FRDGBuilder& GraphBuilder, FRDGBufferRef Buffer, ArrayType&& InArray, int64 Offset = 0)
	{
		...
		const TSharedRef<FArrayWrapper> ArrayWrapper = MakeShared<FArrayWrapper>();
...
public:
	...
	static void KeepAlive(FRDGBuilder& GraphBuilder, TFunction<void()> Lambda);

	template<typename T>
	static void KeepAlive(FRDGBuilder& GraphBuilder, const TSharedPtr<T>& Data)
	{
		FVoxelRenderUtilities::KeepAlive(GraphBuilder, [Data]
		{
			(void)Data;
		});
	}
