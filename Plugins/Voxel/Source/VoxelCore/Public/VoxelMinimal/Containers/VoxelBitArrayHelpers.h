...
#define ShiftOperand(X) (X)
	...
	FORCEINLINE static bool Get(const uint32* RESTRICT Data, uint32 Index)
	{
		const uint32 Mask = 1u << (Index % NumBitsPerWord);
		return Data[Index / NumBitsPerWord] & Mask;
	}
	FORCEINLINE static void Set(uint32* RESTRICT Data, uint32 Index, bool bValue)
	{
		const uint32 Mask = 1u << (Index % NumBitsPerWord);
...
public:
	...
	FORCEINLINE static void ForAllSetBits(const uint32* RESTRICT Data, int32 NumWords, LambdaType Lambda)
	{
