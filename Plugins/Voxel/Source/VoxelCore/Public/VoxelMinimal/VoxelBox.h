...
struct VOXELCORE_API FVoxelBox
{
	...
	FORCEINLINE explicit FVoxelBox(double Min, const FVector3d& Max)
		: FVoxelBox(FVector3d(Min), Max)
	{
	}
	FORCEINLINE explicit FVoxelBox(const FVector3d& Min, double Max)
		: FVoxelBox(Min, FVector3d(Max))
	{
	}
	FORCEINLINE explicit FVoxelBox(FVector3f Min, FVector3f Max)
		: FVoxelBox(FVector3d(Min), FVector3d(Max))
	{
	}
	FORCEINLINE explicit FVoxelBox(FIntVector Min, FIntVector Max)
		: FVoxelBox(FVector3d(Min), FVector3d(Max))
	{
	}
	FORCEINLINE explicit FVoxelBox(double Min, double Max)
		: FVoxelBox(FVector3d(Min), FVector3d(Max))
	{
	}
	...
	FORCEINLINE bool Contains(double X, double Y, double Z) const
	{
		return
			(Min.X <= X) && (X <= Max.X) &&
			(Min.Y <= Y) && (Y <= Max.Y) &&
	...
	FORCEINLINE FVoxelBox Scale(double S) const
	{
		return { Min * S, Max * S };
	}
	FORCEINLINE FVoxelBox Extend(double Amount) const
	{
...
struct FVoxelOptionalBox
{
	...
	FVoxelOptionalBox(const FVoxelBox& Box)
		: Box(Box)
		, bValid(true)
	{
	}
