...
VOXEL_CONSOLE_VARIABLE(
	VOXELRUNTIME_API, bool, GVoxelShowMeshSections, false,
	"voxel.mesh.ShowMeshSections",
	"If true, will assign a unique color to each mesh section");

VOXEL_CONSOLE_VARIABLE(
	VOXELRUNTIME_API, bool, GVoxelDisableStaticPath, false,
	"voxel.mesh.DisableStaticPath",
	"");

VOXEL_CONSOLE_VARIABLE(
	VOXELRUNTIME_API, bool, GVoxelDisableOcclusionCulling, false,
...
void FVoxelMeshSceneProxy::CreateRenderThreadResources()
{
	...
	Mesh->CallInitialize_RenderThread(GetScene().GetFeatureLevel());
}
...
void FVoxelMeshSceneProxy::GetDynamicMeshElements(const TArray<const FSceneView*>& Views, const FSceneViewFamily& ViewFamily, uint32 VisibilityMap, FMeshElementCollector& Collector) const
{
	...
	FMaterialRenderProxy* MaterialProxy = MaterialObject->GetRenderProxy();

	if (GVoxelShowMeshSections)
	{
		const uint32 Hash = FVoxelUtilities::MurmurHash64(uint64(&Mesh.Get()));
		MaterialProxy = new FColoredMaterialRenderProxy(GEngine->LevelColorationLitMaterial->GetRenderProxy(), ReinterpretCastRef<const FColor>(Hash));
...
bool FVoxelMeshSceneProxy::ShouldUseStaticPath(const FSceneViewFamily& ViewFamily)
{
	return
