...
public:
	...
	virtual bool IsUnloadedStructAllowed(const FStructViewerInitializationOptions& InInitOptions, const UE_501_SWITCH(FName, FSoftObjectPath&) InStructPath, TSharedRef<FStructViewerFilterFuncs> InFilterFuncs) override
	{
...
FVoxelInstancedStructDataDetails::FVoxelInstancedStructDataDetails(TSharedPtr<IPropertyHandle> InStructProperty)
{
	...
	check(StructProp);
	check(StructProp->Struct == FVoxelInstancedStruct::StaticStruct());
#endif
...
void FVoxelInstancedStructDetails::CustomizeHeader(TSharedRef<IPropertyHandle> StructPropertyHandle, FDetailWidgetRow& HeaderRow, IPropertyTypeCustomizationUtils& StructCustomizationUtils)
{
	...
	BaseScriptStruct = nullptr;
	{
		...
		{
			BaseScriptStruct = UE_501_SWITCH(FindObject<UScriptStruct>(ANY_PACKAGE, *BaseStructName), UClass::TryFindTypeSlow<UScriptStruct>(BaseStructName));
			if (!BaseScriptStruct)
			{
...
{
	TSharedRef<FVoxelInstancedStructDataDetails> DataDetails = MakeShared<FVoxelInstancedStructDataDetails>(StructProperty);
...
void FVoxelInstancedStructDetails::OnStructPicked(const UScriptStruct* InStruct)
{
	if (StructProperty && StructProperty->IsValidHandle())
	{
		...
		{
			InstancedStruct.InitializeAs(VOXEL_CONST_CAST(InStruct));
		});
