...
#define VOXEL_INITIALIZE_STYLE(Name) \
	...
	{ \
		const TSharedRef<FVoxel ## Name ## Style> Style = MakeShared<FVoxel ## Name ## Style>(); \
		VOXEL_CONST_CAST(FVoxelEditorStyle::Get()).InitModule(*Style, MakeLambdaDelegate([]() -> TSharedPtr<FVoxelStyleSet> \
		{ \
