...
static unsigned char* encodeBytesGroup(unsigned char* data, const unsigned char* buffer, int bits)
{
	...
	checkVoxelSlow(kByteGroupSize % byte_size == 0);
...
{
	checkVoxelSlow(buffer_size % kByteGroupSize == 0);
	...
	for (size_t i = 0; i < buffer_size; i += kByteGroupSize)
	{
		...
		checkVoxelSlow((1 << bitslog2) == best_bits);
		...
		checkVoxelSlow(data + best_size == next);
		data = next;
	}
...
{
	checkVoxelSlow(vertex_count > 0 && vertex_count <= kVertexBlockMaxSize);

	unsigned char buffer[kVertexBlockMaxSize];
	checkVoxelSlow(sizeof(buffer) % kByteGroupSize == 0);
...
#define NEXT(bits) enc = byte >> (8 - bits), byte <<= bits, encv = *data_var, *buffer++ = (enc == (1 << bits) - 1) ? encv : enc, data_var += (enc == (1 << bits) - 1)
	...
	case 3:
		...
		return data + kByteGroupSize;
	default:
		checkVoxelSlow(!"Unexpected bit length"); // unreachable since bitslog2 is a 2-bit value
		return data;
	}
...
{
	checkVoxelSlow(buffer_size % kByteGroupSize == 0);
	checkVoxelSlow(kByteGroupSize == 16);
...
size_t meshopt_encodeVertexBuffer(unsigned char* buffer, size_t buffer_size, const void* vertices, size_t vertex_count, size_t vertex_size)
{
	...
	checkVoxelSlow(vertex_size > 0 && vertex_size <= 256);
	checkVoxelSlow(vertex_size % 4 == 0);
	...
	*data++ = (unsigned char)(kVertexHeader | version);
	...
	checkVoxelSlow(data >= buffer + tail_size);
	checkVoxelSlow(data <= buffer + buffer_size);
...
{
	checkVoxelSlow(unsigned(version) <= 0);
...
#if defined(SIMD_SSE) || defined(SIMD_NEON) || defined(SIMD_WASM)
	checkVoxelSlow(gDecodeBytesGroupInitialized);
